...
In this configuration, the red wire is I2C clock (SCL), the green wire is I2C data (SDA), the orange wire is I2C interrupt, and the yellow wire is ground (GND).  Also note that PB24 is used for I2C interrupts so the jumper J9 must be removed on the MSPM0 LaunchPad.
...
The device will handle a specific interrupt event *New Contract as Consumer*. The process is if the flag newContractCons is set, read the ***Active RDO (Requested Data Object) (0x35)*** register, then write the newContractCons flag location to the ***Interrupt Clear for I2C1 (0x18)*** to clear ONLY that specific flag from the events. 
```c
...
Note sometimes there are extra NAKs when the device is booting up and is not ready to receive I2C traffic. Checking the interrupt event status will ensure the device is booted and ready to receive I2C target commands. 
